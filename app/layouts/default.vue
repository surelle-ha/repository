  ...
  <div class="flex flex-col min-h-screen bg-bg text-ink font-sans antialiased">
    ...
    <header class="sticky top-0 z-50 flex items-center justify-between px-6 h-14
      ...
      <nav class="flex items-center gap-1">
        ...
        <!-- Docs link — hidden when hideDocs is true in DB settings -->
        <NuxtLink v-if="!isDocsHidden" to="/docs"
          class="px-3 py-1.5 rounded-md font-mono text-xs text-ink2 hover:text-ink hover:bg-surface2 transition-all"
          active-class="!text-accent">
          Docs
        </NuxtLink>
        ...
        <button v-if="user" @click="logout"
          class="px-3 py-1.5 rounded-md font-mono text-xs text-muted hover:text-ink
                 hover:bg-surface2 transition-all cursor-pointer">
          Sign out
        </button>
      </nav>
    </header>
...
useHead({ bodyAttrs: { class: 'minim-scroll' } })
