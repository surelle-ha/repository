<template>
  <div class="relative flex flex-col gap-2.5 bg-surface border border-border rounded-xl p-5
    ...
    <!-- Status badge -->
    <div>
      <span class="inline-block px-2 py-0.5 rounded font-mono text-[10px] font-semibold
                   uppercase tracking-widest"
            :class="{
              'bg-green-500/15 text-green-400':  project.status === 'live',
              'bg-yellow-400/12 text-yellow-400': project.status === 'coming_soon',
              'bg-blue-300/12 text-blue-300':    project.status === 'wip',
              'bg-white/7 text-muted':           project.status === 'archived',
            }">
        {{ statusLabel }}
      </span>
    </div>
    ...
    <div v-if="project.tags?.length" class="flex flex-wrap gap-1.5">
      <span v-for="tag in project.tags" :key="tag"
            class="font-mono text-[10px] text-muted tracking-wide">
        #{{ tag }}
      </span>
    </div>
